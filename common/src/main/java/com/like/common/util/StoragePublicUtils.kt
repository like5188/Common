 ...
 *                  3、在应用的自定义界面中显示结果。
 *              使用系统选择器：
 *                  1、使用 ACTION_GET_CONTENT intent，它会要求用户选择要导入的图片。
 *                  2、如果您想过滤系统选择器提供给用户选择的图片类型，您可以使用 setType() 或 EXTRA_MIME_TYPES。
 ...
 *              1、使用 preserveLegacyExternalStorage 标记保留旧版存储模型，以便在用户升级到以 Android 11 为目标平台的新版应用时，应用可以迁移用户的数据。
 ...
 *              2、继续停用分区存储，以便您的应用可以继续在搭载 Android 10 的设备上访问旧版存储位置中的文件。
 ...
 *              1、小文件或包含敏感信息的文件：请使用 Context#getCacheDir()。
 *              2、大型文件或不含敏感信息的文件：请使用 Context#getExternalCacheDir()。
 * 3、暂时停用分区存储
